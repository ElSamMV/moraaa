...
const CAMPOS_ACTUALIZABLES = ['nombre_equipo', 'pais', 'ciudad', 'estadio', 'fundacion', 'entrenador', 'id_admin'];
...
router.get('/', verifyToken, (req, res) => {
    ...
    if (string) {
        whereClause = `WHERE e.id_equipo LIKE ? 
                       OR e.nombre_equipo LIKE ? 
                       OR e.pais LIKE ? 
                       OR e.ciudad LIKE ? 
                       OR e.entrenador LIKE ?`;
        const searchTerm = `%${string}%`;
        queryParams.push(searchTerm, searchTerm, searchTerm, searchTerm, searchTerm);
    }

    // 1. Consulta para obtener el total de registros para la paginación
    const countQuery = `SELECT COUNT(*) AS total FROM equipo e ${whereClause}`;
    ...
    db.query(countQuery, queryParams, (err, countResult) => {
        ...
        const equiposQuery = `SELECT e.*, la.nombre_completo AS nombre_admin 
                               FROM equipo e 
                               LEFT JOIN login_administrador la ON e.id_admin = la.id 
...
router.get('/:id', verifyToken, (req, res) => {
    const { id } = req.params;
    const query = `SELECT e.*, la.nombre_completo AS nombre_admin 
                    FROM equipo e 
                    LEFT JOIN login_administrador la ON e.id_admin = la.id 
                    WHERE e.id_equipo = ?`;
